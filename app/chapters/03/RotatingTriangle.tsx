  ...
  */
  ...
  useEffect(() => {
    ...
    const animate = (currentAngle: number) => {
      const now = Date.now()
      const elasped = now - g_last
      g_last = now
      const newAngle = currentAngle + (ANGLE_STEP * elasped) / 1000.0
      return newAngle % 360
    }

    const draw = (gl: WebGLRenderingContext, currentAngle: number, n: number, modelMatrix: Matrix4, u_ModelMatrix: WebGLUniformLocation) => {
    ...
    const tick = () => {
      ...
      draw(gl, currentAngle, n, modelMatrix, u_ModelMatrix!)
